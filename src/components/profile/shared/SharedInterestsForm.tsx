...
export const InterestsInfoSchema = z.object({
  interests: z
    ...
    .refine(
      (interests) =>
        new Set(interests.map((i) => i.prompt)).size === interests.length,
  ...
  const validateField = (
    ...
    try {
      ...
      if (e instanceof z.ZodError) {
        setErrors((prev) => ({
          ...prev,
          [`${field}_${index}`]: e.errors[0].message,
        }));
      }
    }
  };
...
export function validateInterestsForm(interestsInfo: InterestInfo): {
  isValid: boolean;
