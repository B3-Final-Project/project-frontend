...
export const mapBoosterToProfileCardType = (
  ...
  return {
    ...
    work: booster.work || "",
    description:
      booster.work || `Découvre ${booster.name || "cette personne"} !`,
    languages: booster.languages || [],
    smoking:
      typeof booster.smoking === "number"
        ? formatSmokingEnum(booster.smoking)
        : String(booster.smoking || ""),
    drinking:
      typeof booster.drinking === "number"
        ? formatDrinkingEnum(booster.drinking)
        : String(booster.drinking || ""),
    zodiac:
      typeof booster.zodiac === "number"
        ? formatZodiacEnum(booster.zodiac)
        : String(booster.zodiac || ""),
    interests:
      booster.interests?.map((interest) => ({
        id:
          typeof interest.id === "string"
            ? parseInt(interest.id, 10) || 0
            : Number(interest.id),
        name: interest.name,
        icon: interest.icon || "sparkles",
      })) || [],
    rarity:
      typeof booster.rarity === "number"
        ? booster.rarity === 0
          ? "COMMON"
          : booster.rarity === 1
            ? "UNCOMMON"
