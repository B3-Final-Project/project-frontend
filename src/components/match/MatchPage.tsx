...
const MatchPage = () => {
  ...
  // Fonction pour ouvrir un pack et générer de nouveaux profils
  const handlePackOpened = () => {
    ...
    // Après l'animation des cartes, démarrer l'animation de transition
    setTimeout(() => {
      setShowTransition(true);

      // Après l'animation de transition, afficher le système de match
      setTimeout(() => {
        setShowMatchSystem(true);
        setShowTransition(false);
        setShowCardAnimation(false);
      }, 2000); // Durée de l'animation de transition
    }, 3000); // Durée de l'animation des cartes
  };

  // Fonction pour revenir à l'ouverture de packs
          ...
          <div className="w-full max-w-4xl h-[60vh] flex items-center justify-center">
            {showMatchSystem ? (
              ...
              <div className="relative w-full h-full flex items-center justify-center">
                {/* Animation des cartes qui glissent vers le centre pour former une pile */}
                {packProfiles.map((profile, index) => (
                  <motion.div
                    key={profile.id}
                    className="absolute w-[280px] sm:w-[320px] md:w-[350px]"
                    initial={{
                      x: `calc(${index * 40 - 80}px)`,
                      y: "40%",
                      top: "40%",
                      translateY: "-50%",
                      rotateZ: `${(index - 2) * 5}deg`,
                    ...
                    animate={{
                      ...
                      translateY: "-50%",
                      rotateZ: 0,
                      scale: [1, index === 2 ? 1 : 0.95],
                      opacity: index === 2 ? 1 : [1, 0],
                      zIndex: index === 2 ? 10 : 5 - index
                    }}
                    transition={{
                      duration: 1.2,
                      ease: "easeInOut",
                ...
                {/* Animation de la pile qui se forme au centre */}
                <motion.div
                  className="absolute top-[40%] left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[280px] sm:w-[320px] md:w-[350px] h-[420px] sm:h-[480px] md:h-[520px]"
                  initial={{ opacity: 0 }}
                ...
                >
                  <div className="relative w-full h-full">
                    ...
                    {[0, 1, 2].map((i) => (
                      <div
                        key={i}
                        className="absolute top-0 left-0 w-full h-full rounded-xl bg-gradient-to-br from-[#FF6B6B] via-[#ED2272] to-[#6A5ACD] shadow-xl"
                        style={{
